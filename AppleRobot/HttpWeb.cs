...
namespace AppleRobot
{
	public static class HttpWeb
	{
		public static bool CheckHttps(HttpWebRequest webrequest)
		{
			try
			{
				if (webrequest.RequestUri.Scheme == "https")
				{
					...
					{
						throw new Exception("證書無效，請確保證書存在且有效");
					}
		...
		public static string GetSMSCodeByServerJSON(string sContent)
		{
			/*string imageBase64 = "";
			var o = JObject.Parse(sContent);
			foreach(JToken child in o.Children()) {
				var property = child as JProperty;
				
				if(property != null && property.Value.ToString().IndexOf("data:image/png;base64") > -1) {
					imageBase64 += property.Name + "!" + property.Value.ToString().Replace("data:image/png;base64,", "") + "|";
				}
			}
			
			return imageBase64;*/
			
			smsJSON sms = JsonConvert.DeserializeObject<smsJSON>(sContent);
			string base64Code = sms.IRSV141417879720141024.Replace("data:image/png;base64,", "");
			return base64Code;
		}
		public class smsJSON
		{
			public bool firstTime { get; set; }
			public string IRSV141417879720141024 { get; set;}
		...
		public static string GetHtml(string URL, out string cookie, ref CookieContainer cCon, ref WebHeaderCollection sHeaders, out string lasturl)
		{
			...
			try
			{
				...
				if (response != null)
				{
					text2 = response.Headers.Get("Location");
				}
				if (text2 != null && text2.Length > 0)
				{
					text = HttpWeb.GetHtml(text2, out cookie, ref cCon, ref sHeaders, out lasturl);
					httpWebRequest.GetResponse().Close();
					result = text;
					return result;
				}
				text = new StreamReader(response.GetResponseStream(), Encoding.GetEncoding("utf-8")).ReadToEnd();
				httpWebRequest.GetResponse().Close();
		...
		{
			Stream stream = new MemoryStream(HttpWeb.PostHtmlByBytes(server, URL, byteRequest, cookie, out header));
			StreamReader streamReader = new StreamReader(stream, Encoding.GetEncoding("utf-8"));
		...
		public static byte[] PostHtmlByBytes(string server, string URL, byte[] byteRequest, string cookie, out string header)
		{
			...
			byte[] result = new byte[httpWebResponse.ContentLength];
